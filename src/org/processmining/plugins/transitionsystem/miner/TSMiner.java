...
public class TSMiner {
	...
	public TSMinerOutput mine(final TSMinerInput settings) {
		...
		context.getProgress().setCaption("Constructing initial transition system");
		 ...
		 * Conversion stage.
		 */
		context.getProgress().setCaption("Converting transition system");
		...
		context.addConnection(new TransitionSystemConnection(output.getTransitionSystem(), output.getWeights(), output
				.getStarts(), output.getAccepts(), useSettings ? settings : null));
	 ...
	 */
	public XLog filter(final TSMinerInput settings, TransitionSystem transitionSystem,
			Collection<State> selectedStates, Collection<Transition> selectedTransitions, int treshold) {
		/*
		...
		for (XTrace trace : TSMinerLog.getTraces(log)) {
			...
			if ((states.size() + transitions.size() + treshold) > (selectedStates.size() + selectedTransitions.size())) {
				/*
